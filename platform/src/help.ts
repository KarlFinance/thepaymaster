...
export function adminHelp(): string {
  ...
  ${q("Agreements",
      "Every transaction carries two documents generated from the record: the Sender's Paymaster Agreement (with Transaction, Distribution and Client Account schedules filled in) and, for each recipient, a Recipient's Authorisation carrying the account or wallet they entered. Each party signs in their own account by typing their name against the document; the signature is over a hash of the exact text, and a signed PDF goes into the documents, shared with the party, and into their folder. If an amount, recipient or account changes afterwards, the line in the Agreements panel goes stale and the readiness gate closes until they sign again — Ask to sign again emails them. The gate line 'Every party has signed their agreement' applies to every transaction type.")}
  ${q("Conversions: fiat in, crypto out and crypto in, fiat out",
      "The desk stage. Fiat in, crypto out: the sender pays the client account (manual fiat receipt); on the Conversion panel you instruct MAS Digital with the gross less our 1% — every party is emailed that we have stepped out — and when the desk confirms you record what came back with the hash of its delivery into our client wallet, which is verified on the chain, plus the rate and the desk's 2.5% at source (the desk's charge, never ours). Recipients' amounts become their share of what came back, and Distribute pays them from the client wallet. Crypto in, fiat out is the mirror: the sender sends to our client wallet (verified by hash from a proved wallet), you instruct the sale, record the fiat proceeds with the desk's confirmation, and the manual fiat flow pays recipients from the client account. The Agreement covers the first (Mode B); Keirón is drafting the contract for the second.")}
  ${q("Crypto: Mode C and the sender-executes route",
      "Two routes for a crypto-to-crypto distribution, chosen per transaction under Chain settings (the default for new ones is on the Fiat page). Mode C, as the Agreement describes: the sender sends to one of our registered client wallets — a reference transaction of one unit first, then the balance — and presses 'I have sent it' with the hashes; you record the receipt on the settlement page by hash, and the platform verifies on the chain that it pays our client wallet that amount from one of the sender's proved wallets; then Distribute (linked from the transaction) pays every recipient and our fee from the client wallet, each payment signed in your browser from the wallet that holds the key and verified before it is recorded. The readiness line 'The client wallet is one of ours and its control is proved' must be green. The other route: the sender signs every payment from their own proved wallet on their Send page and nothing is held by us. Assets: USDT, USDC and Ether on Ethereum, Bitcoin, USDT on Tron (TRC-20, TronLink). Set TRONGRID_API_KEY as a secret for a higher Tron rate limit.")}
  ${q("ThePaymaster's wallets",
      "Wallets, in the rail: the registry of our own addresses — a fee wallet per rail, and later client wallets for Mode C. A transaction's fee wallet is chosen from this list under Chain settings; it cannot be typed. Each wallet is proved by a signature from its own key (Show the message to sign, sign it in the wallet, Record it), exactly as recipients prove theirs. The readiness line 'Our fee goes to a registered ThePaymaster wallet' fails if a transaction points anywhere else. Owners can retire a wallet with a reason; nothing is deleted. Private keys never touch the platform.")}
  ${q("Fiat: the manual flow",
      "Until HSBC issues API credentials, fiat is confirmed by people. The Bank operations panel shows the six steps and where the transaction is: agreements signed; the sender pays the client account under the Reference Code and presses 'I have sent it' (you are emailed); you confirm receipt on the settlement page with the bank advice as evidence (everyone is emailed); you pay each recipient — the payment file gives you the rows — and record each payment with evidence on the settlement page (the recipient is emailed and asked to confirm); each recipient presses 'It has arrived' in their account; you seal. The Fiat page in the rail lists every fiat transaction with its stage, and holds the mode switch: Manual today, HSBC API when credentials exist, sender-direct built but off.")}
  ...
  ${q("Fiat: the sender pays directly",
      "On a fiat-to-fiat transaction the Bank operations panel asks who makes the payments. Mandated account (the default): the sender pays us, we pay everyone. Sender pays directly: nothing passes through an account we operate — the sender downloads the payment file from their page (every recipient plus our fee, each with its reference), uploads it to their own bank, then pastes their statement into their page; the matching lines become the record, the holder throughout is the client, and the certification says the funds were never held by us. Choose before anything is paid; the choice is locked once a payment is recorded. Set FEE_BANK_ACCOUNT in wrangler.toml so the fee row carries our account.")}
