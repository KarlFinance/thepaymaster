...
export async function staffSenderSaysSent(env: Env, actor: Actor, txId: string, note: string): Promise<void> {
  await quietly("sender says sent", async () => {
    ...
    await send(env, actor, {
      ...
      text: [
        `${s?.display_name ?? "The sender"} has marked ${t.ref} as sent to the client account or client wallet.`,
        note ? `\nIn their words: "${note}"\n` : ``,
        `Check for the Reference Code ${t.ref} (or the transaction hash they gave), then record the receipt with its evidence on the settlement page:`,
        ``,
        `${ADMIN}/t/${t.id}/settle`,
      ].join("\n"),
      about: { kind: "transactions", id: txId },
